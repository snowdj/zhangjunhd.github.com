...
1. 棣莫弗的研究的动因
    ...
    * 由此得出`\(lim_{n \to \infty}K_N=0\)`，再由`\(P(|\frac{X}{N}-p|≥\epsilon)≤\epsilon^{-1}K_N\)`(J注:?)，即得`\(lim_{n \to \infty}P(|\frac{X}{N}-p|≥\epsilon)=0\)`，这就是伯努利大数定律。
2. 棣莫弗的初步结果
...
2. 贝叶斯的问题提法
    * 贝叶斯的问题可表述为：设X服从二项分布`\(B(N,\theta)\)`，N已知而`\(\theta\)`未知。给定常数a，b，0≤a<b≤1，在得到观察值之后，要求条件概率`\(P(a≤\theta≤b|X)\)`。
    * 例如，为证明男婴的概率`\(\theta > \frac{1}{2}\)`，在所观察的N个出生婴儿中发现男婴X个，要由此计算`\(P(\frac{1}{2}<\theta≤1|X)\)`。
3. 贝叶斯假设
    * 要计算`\(\theta\)`的条件分布，就必须知道`\(\theta\)`的无条件分布——即在观察到X的值以前`\(\theta\)`有如何分布？
    * “台球模型”：有一张矩形的台球桌，不妨设其长边为1.有甲乙两人，甲先向桌上抛一球，使此球落在桌面上任何一处有同等机会。记A的横坐标为`\(\theta\)`，则由A服从台面上的均匀分布，可知`\(\theta\)`服从[0,1]区间上的均匀分布R(0,1)。过A作一直线垂直于桌面的长边，它与长边之交点即为`\(\theta\)`。然后甲再向台面上抛掷N个球(图中X)，每个球的位置服从台面上的均匀分布且各次抛掷独立，甲数清这N个球中，处在虚线左边的个数为X。甲把N和X两个数据告诉乙。甲要求乙依据N和X去估计`\(\theta\)`落在一指定区间[a,b]内的概率。![1](/assets/2014-02-21-history-of-statistics/1.png)
    * 贝叶斯把“对`\(\theta\)`之值一无所知”这一含义不精确的说法，通过这个模型的直观上的观照，数学化为`\(\theta\)`有R(0,1)均匀分布这个确切陈述。后人把“`\(\theta\)`有均匀分布R(0,1)”这一陈述称为`贝叶斯陈述`，也称为`“同等无知”假设`，`\(\theta\)`的分布R(0,1)称为`先验分布`。即`\(\theta\)`的`无条件分布`。
    * 费歇尔提出：若对`\(\theta\)`一无所知，那么对`\(\theta\)`的一个函数，例如`\(\theta^2\)`也应一无所知，故按贝叶斯假设，`\(\theta^2\)`也应该有均匀分布R(0,1)，这就与`\(\theta \sim R(0,1)\)`产生了矛盾。
4. 问题的解答
...
5. 贝叶斯假设的另一种解释
    * 基于(1)式，此式对X取0,1,...,N，这N+1个可能值中的每一个给以等概率`\((N+1)^{-1}\)`，他认为这个结果是对`\(\theta\)`“绝对一无所知”的一种合理解释，而这一假设，正是贝叶斯假设`\(\theta \sim R(0,1)\)`的直接后果。
6. 拉普拉斯的不充分推理原则
...
7. 贝叶斯统计学
    * 统计推断一般的模式是：样本X的分布或概率密度`\(f_\theta(x)\)`依赖于未知参数`\(\theta\)`，只知道`\(\theta\)`属于某一集合`\(\Theta\)`，但不知它取`\(\Theta\)`中的何值。统计推断的任务，就是依据所抽得的样本X去作出某种有关`\(\theta\)`之值的论断，例如对`\(\theta\)`的值作出估计，或判断`\(\theta\)`是否落在`\(\Theta\)`的某个指定子集`\(\Theta_1\)`之内等。
    * `频率学派`其特点是把需要推断的参数`\(\theta\)`视为固定的未知常数而样本X为随机的，其着眼点在样本空间，有关的概率计算都是针对X的分布，另一叫`贝叶斯学派`，其特点正好与上述相反，参数`\(\theta\)`视为随机变量而样本X视为固定的，其着眼点在参数空间，重视的是参数`\(\theta\)`的分布。
    * 估计二项分布概率`\(\theta\)`的问题：
        * 频率学派：以X记事件在N次观察中出现的次数，用频率X/N估计`\(\theta\)`，其与`\(\theta\)`的接近程度，可以用方差`\(E(\frac{X}{N}-\theta)^2\)`去衡量，这求期望的运算是针对X的分布，`\(\theta\)`始终看作固定，没有随机性。
        * 贝叶斯学派：对`\(\theta\)`的了解是具有先验分布R(0,1)，经过样本X的信息加入后，把对`\(\theta\)`的了解调整为`\(\beta分布：(N+1)C_X^N\theta^X(1-\theta)^{N-X}\)`,对比图为![2](/assets/2014-02-21-history-of-statistics/2.png)
...
1. 从算术平均谈起
    ...
    * 使误差平方和达到最小以寻求估计值的方法，就叫做最小二乘法。一般形式可表述为：`\(目标函数=\sum(观测值-理论值)^2\)`
2. 勒让德以前的有关研究
