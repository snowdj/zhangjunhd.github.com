...
2. 棣莫弗的初步结果
    * 假定N为偶数2m，概率p=1/2，记b(i)=b(2m,1/2,i),中心项与一个任意项之比b(m)/b(m+d)
    * 当N->∞时，
        * `\(b(m)~2.168\frac{(1-\frac{1}{N})^N}{\sqrt{N-1}}\)`(5)
        * `\(log(\frac{b(m)}{b(m+d)})~(m+d-\frac{1}{2})log(m+d-1)+(m-d+\frac{1}{2})-2mlogm+log(\frac{m+d}{m})\)`(6)
3. 初步结果的改进与斯特林的联系
    ...
    * (8)右边只取主项1，即得到`\(b(m)~\sqrt{\frac{2}{\pi N}}\)`(9)
    ...
    * 斯特林公式(1730)
        ...
        * 教科书上常见的形式`\(m!~\sqrt{2\pi}m^{m+\frac{1}{2}}e^{-m}\)`(12)
4. 积分形式`\(P_d\)`的近似公式
    * 1733进一步证明N->∞,`\(\frac{b(m+d)}{b(m)}~exp(-\frac{2d^2}{N})\)`(13)
    * (9)和(13)结合，`\(b(m+d)~\frac{2}{\sqrt{2\pi N}e^{-\frac{2d^2}{N}}}\)`(14)
    * 利用(14)，并近似地以定积分代替和，得
5. 棣莫弗工作统计意义的讨论 
    * (17)说明了：就“用频率估计概率”这个特例而言，观察值的算术平均(在此例即频率)的精度，与观察次数N的平方根`\(\sqrt{N}\)`成比例。棣莫弗也看出了`\(\sqrt{N}\)`这个量的特殊地位，他为此引进“摸”(modulu)这个称呼。它被现在常用的概念标准差所取代了。
