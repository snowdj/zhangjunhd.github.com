...
1. 对于任意事件E和F，已知F发生的条件下，E发生的[条件概率][11]记为P(E|F)，定义如下`\(P(E|F) = \frac{P(EF)}{P(F)}\)`
2. 等式`\(P(E_1E_2 ... E_n) = P(E_1)P(E_2|E_1) ... P(E_n|E_1 ... E_n-1)\)`称为概率的[乘法规则][12]。
3. 一个有用的等式 `\(P(E) = P(E|F)P(F) + P(E|F^c)P(F^c)\)`可以来通过以F是否发生为条件计算P(E)。此公式为[全概率公式[13]。
...
5. 令`\(F_i = 1, ..., n\)`为互不相容事件列，且它们的并为整个样本空间，等式`\(P(F_j|E) = \frac{P(E|F_j)P(F_j)}{\sum_{i=1}^n{P(E|F_i)P(F_i)}}\)`称为[贝叶斯公式][14]。如果事件`\(F_i = 1, ..., n\)`为一组假设，那么贝叶斯公式说明了如何计算当新证据成立时，这些假设成立的条件概率。
6. 如果P(EF) = P(E)P(F)，那么我们称事件E和F是[独立][15]的。该等式等价于P(E|F) = P(E)或P(F|E) = P(F)。也即，如果知道其中之一的发生并不影响另一个的发生的概率，那么E和F独立。
    * 如果E和F是一次试验中的两个互不相容事件，那么在独立重复试验时，事件E在事件F之前发生的概率为`\(\frac{P(E)}{P(E) + P(F)}\)`
    * 称事件E1和E2对于给定的事件F是`条件独立`的(conditionally independent)，如果已知F发生的条件下，E1发生的概率不因E2是否发生而改变，即
        * P(E1|E2F)=P(E1|F)
        * 或等价地P(E1E2|F)=P(E1|F)P(E2|F)

##第四章 随机变量

1. 定义在试验结果上的实值函数称为[随机变量][16]。如果X是一个随机变量，那么如下定义的函数F(X) = P{X ≤ x} 称为随机变量的[分布函数][17]。任意有关X的概率可以通过F进行计算。
2. 若一个随机变量的可能取值的集合是有限集，或者可数无限集，那么称该随机变量为[离散型随机变量][18]。如果X是一个离散型随机变量，那么函数p(x) = P{X = x}称为X的概率分布列或分布列。
    * 离散型随机变量的分布函数可通过分布列p(a)计算：`\(F(a)=\sum_{x:x≤a}p(x)\)`
3. 如下定义的`\(E[X] = \sum_{x:p(x) > 0}{xp(x)}\)`称为X的[期望值][19]，E[X]通常也称为X的`均值`(mean)或`期望`(expectation)。
    * 设g(x)是一个实值函数，则对于离散型随机变量X，有`\(E[g(X)] = \sum_{x:p(x) > 0}{g(x)p(x)}\)`。
    * 若a和b是常数，则E[aX+b]=aE[X]+b
    * 随机变量和的期望等于它们的期望的和。即`\(E[\sum_{i=1}^n{X_i}] = \sum_{i=1}^n{E[X_i]}\)`
4. 随机变量X的[方差][20]，记为Var(x)，定义为`\(Var(X) = E[(X - E[X])^2]\)`。方差等于X与它的期望的差的平方的期望，它度量了X可能取值的分散程度。
    * `\(Var(X) = E[X^2] - (E[X])^2\)`
    * `\(Var(aX+b) = a^2Var(X)\)`
5. `\(SD(X)=\sqrt{Var(X)}\)`称为X的[标准差][21]。

一些常用的离散型随机变量:

1. 若随机变量X的分布列为`\(p(i) = {n \choose i}p^i(1-p)^{n - i}, i = 0,..., n\)`，则X称为参数为(n,p)的[二项随机变量][22]。该随机变量可解释为n次独立重复试验中试验成功的次数，而每次试验成功的概率为p。它的均值:E[X] = np,方差:Var[X] = np(1-p)。
...
2. 若随机变量X的分布列为`\(p(i) = \frac{e^{-\lambda}\lambda^i}{i!}, i = 0,1,...\)`,则X称为参数为λ的[泊松随机变量][23]。如果进行多次(近似)独立的试验，而且每次成功的概率都较小，那么总的成功次数就近似为泊松随机变量。泊松随机变量的均值和方差都等于其参数λ，也即E[X] = Var[X] = λ。
    * 当n足够大时，p充分小，而使得np保持适当的大小时，以(n,p)为参数的二项随机变量可近似看作泊松分布(即成功的次数近似服从参数为λ=np的泊松分布)。以下例子中的随机变量通常都服从泊松分布:
        * 一本书里一页或若干页中的印刷错误
        * 某地区居民活到100岁的人数
        * 一天中拨错电话号码的总数
        * 一家便利店里每天卖出狗粮饼干的盒数
    ...
    * 泊松分布的另一应用表现在这样的情形中，“事件”发生在某些时间点上。这种事件的例子有：发生一次地震，某人进入特定地点(如银行、邮局、加油站)，爆发一次战争等。我们假设这样的事件发生在一列(随机)时间点上，并设存在某个正的常数λ使得如下条件成立:
        ...
        * 粗略地说，条件1和条件2说明，当h比较小时，在长度为h的区间内正好发生1个事件的概率等于`\(\lambda h\)`加上某个比h更小的变量，而事件发生多于一次的概率就是一个比h更小的量。条件3说明，在一个时间区间内无论发生了什么，对另一个与它不相交的区间(从概率意义上)没有影响。这样，如果事件的发生满足条件1，2，3，则在任何固定的长度为t的时间区间内，事件发生的次数是以`\(\lambda t\)`为参数的泊松随机变量。这时，我们称事件是按速率为λ的泊松过程发生的。数`\(\lambda\)`可解释为单位时间内事件发生的速率。它一定是由经验确定的常数。
...
2. 随机变量X和Y称为`独立的`，如果对任意集合A和B，有`\(P\{X \in A,Y \in B\} = P\{X \in A\}P\{Y \in B\}\)`
    ...
    * 若X和Y为独立连续型随机变量，则它们的和的分布函数可以通过此式得到:`\(F_{X+Y}(a) = \int_{-\infty}^{\infty}{F_X(a-y)f_Y(y)dy}\)`，分布函数`\(F_{X+Y}\)`称为分布函数`\(F_X\)`和`\(F_Y\)`的[卷积][44]
...
[26]:http://science.scileaf.com/library/428
[27]:http://science.scileaf.com/library/341
[28]:http://science.scileaf.com/library/154
[29]:http://science.scileaf.com/library/438
[30]:http://science.scileaf.com/library/158
[31]:http://science.scileaf.com/library/142
[32]:http://science.scileaf.com/library/18
[33]:http://science.scileaf.com/library/1379
[34]:http://science.scileaf.com/library/86
[35]:http://science.scileaf.com/library/2375
[36]:http://science.scileaf.com/library/85
[37]:http://science.scileaf.com/library/164
[38]:http://science.scileaf.com/library/87
[39]:http://science.scileaf.com/library/84
[40]:http://science.scileaf.com/library/602
[41]:http://science.scileaf.com/library/659
[42]:http://science.scileaf.com/library/89
[43]:http://science.scileaf.com/library/90
[44]:http://science.scileaf.com/library/984
[45]:http://science.scileaf.com/library/888
[46]:http://science.scileaf.com/library/2569
[47]:http://science.scileaf.com/library/602
[48]:http://science.scileaf.com/library/89
[49]:http://science.scileaf.com/library/216
[50]:http://science.scileaf.com/library/753
[51]:http://science.scileaf.com/library/582
[52]:http://science.scileaf.com/library/997
[53]:http://science.scileaf.com/library/331
[54]:http://science.scileaf.com/library/436
[55]:http://science.scileaf.com/library/116
[56]:http://science.scileaf.com/library/1121
[57]:http://science.scileaf.com/library/124
